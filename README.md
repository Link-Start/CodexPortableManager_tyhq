...
- 遇到使用问题或发现缺陷，可以在 [GitHub Issues](https://github.com/tyhq/CodexPortableManager/issues) 提交反馈，也可以加入 QQ 群 `535990598` 交流。反馈时建议附上 Windows 版本、管理器版本、操作步骤和界面或日志中的错误信息；公开发送前请隐藏个人路径、账号等敏感内容。
...
- 可选补全中文菜单、显示已接入的外部模型、完整展示模型原始推理、统一推理强度名称，并修正特定 Windows 沙箱账户识别问题。
...
模型返回原始推理内容时优先完整展示，取消推理卡片内部固定高度，并让每个已完成的原始推理块默认保持展开；用户仍可手动折叠。没有原始内容时继续使用官方摘要和默认折叠行为。
